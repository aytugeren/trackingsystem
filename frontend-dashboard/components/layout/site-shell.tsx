...
export function SiteShell({ children }: { children: React.ReactNode }) {
  ...
  const sidebarStyle = { width: collapsed ? 72 : 288, minWidth: collapsed ? 72 : 288 }

  return (
    <div className="grid min-h-dvh grid-cols-1 md:grid-cols-[var(--sidebar-width)_1fr]" style={layoutStyle}>
      {/* Sidebar */}
      <aside
        className={`fixed inset-y-0 left-0 z-50 shrink-0 border-r bg-white/80 backdrop-blur transition-transform duration-200 ease-out md:static md:translate-x-0 dark:bg-background ${open ? 'translate-x-0' : '-translate-x-full'}`}
        style={sidebarStyle}
      >
        <div className="h-14 border-b px-4 flex items-center justify-center font-semibold tracking-tight">
          <span className="text-lg" aria-label="KTP Dashboard">
            {collapsed ? '♦' : 'KTP Dashboard'}
          </span>
        </div>
        <div className="p-3">
          <MainNav collapsed={collapsed} onNavigate={() => setOpen(false)} />
        </div>
      </aside>

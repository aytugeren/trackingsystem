...
class _LeaveRequestPageState extends State<LeaveRequestPage> {
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: AppBar(
        ...
        actions: [IconButton(onPressed: _loadLeaves, icon: const Icon(Icons.refresh))],
      ),
      body: SingleChildScrollView(
        ...
        child: Column(
          ...
          children: [
            Text('Talep Olustur', style: Theme.of(context).textTheme.titleLarge),
            Row(children: [
            ...
            if (_useTime)
              Row(children: [
                Expanded(child: _timeField('Baslangic Saati', _fromTime, () async {
                  final picked = await showTimePicker(context: context, initialTime: _fromTime);
                  if (picked != null) {
                    setState(() {
                      _fromTime = picked;
                      final same = _from.year == _to.year && _from.month == _to.month && _from.day == _to.day;
                ...
                Expanded(child: _timeField('Bitis Saati', _toTime, () async {
            ...
            Row(children: [
              Expanded(child: _dateField('Baslangic', _from, () async {
              ...
              const SizedBox(width: 8),
              Expanded(child: _dateField('Bitis', _to, () async {
            ...
            const SizedBox(height: 8),
            TextField(controller: _reason, decoration: const InputDecoration(labelText: 'Aciklama')),
            const SizedBox(height: 12),
            FilledButton.icon(onPressed: _submitting ? null : _submit, icon: const Icon(Icons.send), label: Text(_submitting ? 'Gonderiliyor...' : 'Gonder')),
  ...
  Widget _timeField(String label, TimeOfDay t, Future<void> Function() onPick) {
    ...
    return OutlinedButton(
      onPressed: onPick,
      child: Row(mainAxisAlignment: MainAxisAlignment.spaceBetween, children: [Text(label), Text(fmt(t))]),
    );
  }

  Future<void> _submit() async {
    if (_to.isBefore(_from)) { _show('Bitis tarihi baslangictan once olamaz'); return; }
    setState(() => _submitting = true);
    try {
      if (_useTime) {
        final fromMin = _fromTime.hour * 60 + _fromTime.minute;
        final toMin = _toTime.hour * 60 + _toTime.minute;
        if (toMin <= fromMin) { _show('Gecersiz saat araligi'); return; }
      }
            ...
            children: [
              Padding(
                ...
                child: Row(children: const [
                  ...
                  SizedBox(width: 160, child: Text('Kisi', style: TextStyle(fontWeight: FontWeight.bold))),
                  SizedBox(width: 80, child: Text('Durum', style: TextStyle(fontWeight: FontWeight.bold))),
                  SizedBox(width: 240, child: Text('Aciklama', style: TextStyle(fontWeight: FontWeight.bold))),
                ]),
              ),
              ...
              ...leaves.map((l) => Padding(
                    ...
                    child: Row(children: [
                      ...
                      SizedBox(width: 80, child: Row(children: [Icon(Icons.circle, size: 12, color: _statusColor(l.status))])),
  ...
  Widget build(BuildContext context) {
    ...
    Color? leaveColor(DateTime d) {
      ...
      for (final l in leaves) {
        ...
        if (s == 'approved') hasApproved = true; else if (s == 'pending') hasPending = true;
      }
      ...
      children: List.generate(rows, (r) {
        return Row(
          children: List.generate(7, (c) {
            ...
            final date = inMonth ? DateTime(month.year, month.month, dayNum) : null;
